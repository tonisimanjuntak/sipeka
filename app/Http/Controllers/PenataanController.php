...
class PenataanController extends Controller
{
    ...
    {
        $this->penataan = new Penataan;
        $this->isLogin();
    }

    public function index($jenispenataan)
    {                
        $data['menu'] = $jenispenataan;
        $data['jenispenataan'] = $jenispenataan;
        return view('penataan.index', $data);
    }

    public function tambah($jenispenataan)
    {
        if (session('akseslevel') != 'Operator Kabupaten') {
            return redirect('penataan')->with('other', 'Hanya Operator Kabupaten yang dapat mengakses!');            
        }
        ...
        $data['jenispenataan'] = $jenispenataan;
        $data['ltambah'] = true;
        return view('penataan.form', $data);
    }
    ...
    public function progress($nopengajuan)
    {
        try {
            ...
            $rsPenataan = Penataan::findOrFail($nopengajuan);
        } catch (ModelNotFoundException $e) {
            return redirect('penataan')->with('other', 'Data tidak ditemukan!');
        }
        $jenispenataan = $rsPenataan->jenispenataan;

        $data['stepNumber'] = App::getStepNumber($rsPenataan->idstatuspengajuanterakhir);
        $data['menu'] = $jenispenataan;
    ...
    {
        // Query dasar
        $query = Penataan::select("*");

        $jenispenataan = $request->input('jenispenataan');
        $query->where('jenispenataan', $jenispenataan);
        ...
        foreach ($rsData as $row) {
            ...
            if ($row->idstatuspengajuanterakhir == '001') {
                $button .= '
                    <a href="' . url('penataan/hapus/' . Crypt::encrypt($row->nopengajuan)) . '" class="dropdown-item" id="btnHapus">Hapus</a>
                ';                
            }
            ...
            $data[] = [
                ...
                'action' => '<div class="btn-group btn-block">
                                ...
                                <a href="' . url('penataan/progress/' . Crypt::encrypt($row->nopengajuan)) . '" class="btn btn-warning">Progress</a>                                
                            </div>',
    ...
    public function simpanPengajuan(Request $request)
    {
        ...
        $jenispenataan = $request->get('jenispenataan');
        ...
        $nopengajuan = $this->penataan->createID();
        ...
        $simpan = $this->penataan->simpanPengajuan($data, $persyaratanDasar, $persyaratanAdministratif, $persyaratanTeknis, $dataKelurahan);
    ...
    public function simpanVerifikasiPropinsi(Request $request)
    {
        ...
        $rsPengajuan = Penataan::find($nopengajuan);

        if ($this->penataan->tanggalStatusInvalid($nopengajuan, $tglverifikasipropinsi)) {
            return response()->json([
        ...
        if ($statusverifikasipropinsi == '1') {
            $idstatuspengajuanterakhir = $idstatuspengajuan;
            $idstatuspengajuannext = $this->penataan->createStatusPenataan($idstatuspengajuan);
        ...
        $idriwayat = $this->penataan->createIDRiwayat($nopengajuan);
        $riwayat = array(
                'idriwayat' => $idriwayat,
                'nopengajuan' => $nopengajuan,
        ...
        $simpan = $this->penataan->simpanVerifikasiPropinsi($data, $riwayat);        
    ...
    public function simpanRaperda(Request $request)
    {
        ...
        if ($this->penataan->tanggalStatusInvalid($nopengajuan, $tglraperda)) {
            return response()->json([
        ...
        $simpan = $this->penataan->simpanRaperda($data, $riwayat);        
    ...
    public function simpanTelaahan(Request $request)
    {
        ...
        if ($this->penataan->tanggalStatusInvalid($nopengajuan, $tglregister)) {
            return response()->json([
                'message' => 'Tanggal raperda tidak boleh lebih kecil dari tanggal verifikasi propinsi!'
            ]);
        }
        ...
        $simpan = $this->penataan->simpanTelaahan($data, $riwayat);        
    ...
    public function simpanPermohonanKode(Request $request)
    {
        ...
        if ($this->penataan->tanggalStatusInvalid($nopengajuan, $tglpermohonankode)) {
            return response()->json([
        ...
        $simpan = $this->penataan->simpanPermohonanKode($data, $riwayat);        
    ...
    public function simpanRekomendasiGubernur(Request $request)
    {
        ...
        if ($this->penataan->tanggalStatusInvalid($nopengajuan, $tglrekomendasigubernur)) {
            return response()->json([
        ...
        $simpan = $this->penataan->simpanRekomendasiGubernur($data, $riwayat);        
    ...
    public function simpanKirimSuratKeKemendagri(Request $request)
    {
        ...
        if ($this->penataan->tanggalStatusInvalid($nopengajuan, $tglkirimsuratkekemendagri)) {
            return response()->json([
        ...
        $simpan = $this->penataan->simpanKirimSuratKeKemendagri($data, $riwayat);        
    ...
    public function simpanSkKemendagri(Request $request)
    {
        ...
        if ($this->penataan->tanggalStatusInvalid($nopengajuan, $tglskkemendagri)) {
            return response()->json([
                'message' => 'Tanggal rekomendasi gubernur tidak boleh lebih kecil dari tanggal permohonan kode!'
            ]);
        }
    ...
    public function hapus($nopengajuan)
    {
        ...
        $rsPembentukanPersyaratanDasar = $this->penataan->getPersyaratanDasar($nopengajuan);
        $rsPembentukanPersyaratanAdministratif = $this->penataan->getPersyaratanAdministratif($nopengajuan);
        $rsPembentukanPersyaratanTeknis = $this->penataan->getPersyaratanTeknis($nopengajuan);

        $hapus = $this->penataan->hapusData($nopengajuan, $rsPenataan);
        if ($hapus['status'] == 'success') {

            if (!empty($rsPenataan->filesuratpengantar)) {
                Uploads::hapusFile('uploads/pengajuan', $rsPenataan->filesuratpengantar);
            }
    ...
    public function getId(Request $request)
    {
        ...
        $rsPenataan = Penataan::find($nopengajuan);
        return response()->json($rsPenataan);
    }
    ...
    public function getPengajuanPenataan(Request $request)
    {
        $nopengajuan = $request->input('nopengajuan');
        $rsPenataan = Penataan::find($nopengajuan);
        $rsDesaTerpilih = $this->penataan->getDesaTerpilih($nopengajuan);
        $rsPersyaratanDasar = $this->penataan->getPersyaratanDasar($nopengajuan);
        $rsPersyaratanAdministratif = $this->penataan->getPersyaratanAdministratif($nopengajuan);
        $rsPersyaratanTeknis = $this->penataan->getPersyaratanTeknis($nopengajuan);

        return response()->json(array('rsPenataan' => $rsPenataan, 'rsDesaTerpilih' => $rsDesaTerpilih, 'rsPersyaratanDasar' => $rsPersyaratanDasar, 'rsPersyaratanAdministratif' => $rsPersyaratanAdministratif, 'rsPersyaratanTeknis' => $rsPersyaratanTeknis));
    }
}
