            ...
            <div class="card shadow mb-4">
                ...
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                    <h6 class="m-0 font-weight-bold text-primary">
                        @switch($jenispenataan)
                        @case('pembentukan')
                        LIST DATA PEMBENTUKAN KECAMATAN
                        @break
                        @case('penggabungan')
                        LIST DATA PENGGABUNGAN KECAMATAN
                        @break
                        @case('penyesuaian')
                        LIST DATA PENYESUAIAN KECAMATAN
                        @break
                        @default
                        -
                        @endswitch
                    ...
                    <a href="{{ url('penataan/tambah/'.$jenispenataan) }}" class="btn btn-sm btn-primary"><i
                            class="fa fa-plus"></i>
                        Tambah Data</a>
                    @endif
                </div>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-12">
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input" type="radio" name="statusFilter" id="semua"
                                            value="Semua" checked>
                                        <label class="form-check-label" for="semua">Semua</label>
                                    </div>
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input" type="radio" name="statusFilter" id="onprogress"
                                            value="Onprogress">
                                        <label class="form-check-label" for="onprogress">On Progress</label>
                                    </div>
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input" type="radio" name="statusFilter" id="selesai"
                                            value="Selesai">
...
<script>
    var table;
    var jenispenataan = "{{ $jenispenataan }}";
    ...
    $(document).ready(function() {
        table = $('#tableIndex').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "{{ url('penataan/listindex') }}",
                type: 'GET',
                data: function(d) {
                    d.statusFilter = $('input[name="statusFilter"]:checked').val();
                    d.jenispenataan = jenispenataan;
                }
            },
            pageLength: 10,
            lengthMenu: [
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            if (response.success) {
                                ...
                                .then(() => {
                                    window.location.href = "{{ url('penataan') }}";
                                });
                            } else {
